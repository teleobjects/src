...
// Set the colour of a gradient
  void setGradient(const byte y, const OLED_Colour colour);
  ...
  int drawChar(const int x, const int y, const char letter, const OLED_Colour colour, const OLED_Colour background, const boolean alpha);
...
#endif
  void drawString(int x, int y, const char *bChars, OLED_Colour foreground, OLED_Colour background, boolean alpha);
  void drawString(int x, int y, const String &str, OLED_Colour foreground, OLED_Colour background, boolean alpha);
  ...
  */
     ...
     {
       // SPI.beginTransaction(SPISettings(960000000, MSBFIRST, SPI_MODE0)); /////////////// 
...
public:
  ...
  inline void setGradient(byte y, OLED_Colour color) { setGradient(y, color); }

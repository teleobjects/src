...
int OLED::drawChar(const int x, const int y, const char letter, const OLED_Colour colour, const OLED_Colour background, const boolean alpha)
{
  ...
  if (c == ' ') {
    ...
    if (!alpha) this->drawFilledBox(x, y, x + charWide, y + header.height - 1, background);
    return charWide;
  }
  uint8_t width = 0;
...
template <class StrType> __attribute__((always_inline)) inline void _drawString(OLED *oled, void*font, int x, int y, StrType str, OLED_Colour foreground, OLED_Colour background, boolean alpha)
{
  if (x > COLUMNS || y > ROWS)
    return;
  ...
  for(int idx = 0; c = str[idx], c != 0; idx++) {
    if(c == '\n') { // Newline
      ...
      int charWide = oled->drawChar(x+strWidth, y, c, foreground, background, alpha);
      if (charWide > 0) {
        strWidth += charWide ;
        if (!alpha) oled->drawLine(x + strWidth , y, x + strWidth , y + header.height-1, background);
        strWidth++;
      } else if (charWide < 0) {
        return;
      }
...
void OLED::drawString(int x, int y, const char *bChars, OLED_Colour foreground, OLED_Colour background, boolean alpha)
{
  _drawString(this, (void*)this->font, x, y, bChars, foreground, background, alpha);
}

void OLED::drawString(int x, int y, const String &str, OLED_Colour foreground, OLED_Colour background, boolean alpha)
{
  _drawString(this, (void*)this->font, x, y, str, foreground, background, alpha);
}

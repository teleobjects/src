...
size_t OLED_TextBox::write(uint8_t character) {
  ...
  if(character == '\n') {
    ...
    oled.drawChar(cur_x+left,cur_y+bottom-rowHeight,character,this->foreground,this->background, true);
    cur_x += char_width;
    if(cur_x < this->width)
...
void OLED_TextBox::scroll(uint8_t rowHeight) {
  ...
  *target = 0;
  ...
  while(*replay) {
    ...
    oled.drawChar(cur_x+left,cur_y+bottom-rowHeight,*replay,this->foreground,this->background, true);
    cur_x += char_width;
    if(cur_x < this->width) {

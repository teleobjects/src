...
class Weather {
  ...
  ArrayList<String> getPages() {
    ...
    if (!updated) {
      ...
      pages.add(createString(cleanUp("IT'S "+nf(metric ? getCelcius(temp) : temp, 0, 1) + (metric ? char(DEGREE)+"c" : char(29)+"f"), false), CENTERED, 1, 1, 10));
      pages.add(createString(cleanUp(int(humidity)+"% HUMID", true), CENTERED, 1, 1, 10));
      pages.add(createString(cleanUp("PRESSURE "+int(pressure)+"mPa", false), CENTERED, 1, 1, 10));
      pages.add(createString(cleanUp(int(clouds)+"% CLOUDY", true), CENTERED, 10, 1, 10));
      pages.add(createString(cleanUp("WIND "+int(windSpeed) +"m/h "+(int)windDeg+char(DEGREE)+" "+getHeading(windDeg), false), CENTERED, 1, 1, 10));

...
public class tele_comments_java extends PApplet {
...
public void play() {
        ...
        if (millis() - lastPage > tuck*100) {
          ...
          if (thisPage.indexOf(TAB+"") != -1) {
            ...
            if (items.length > 3) {
              ...
              writeString(stringText, displayMode, tick, 0, tock);
            }
          } else {
            stringText = thisPage;
            displayMode = SCROLL_ALL_RIGHT;
            tick = 1;
            tock = 0;
            tuck = 15;
            writeString(stringText, displayMode, tick, 0, 15);
          }
          if (play) {
            pageIndex ++;
  ...
  if (channel == TIME) {
    if (millis() - lastPage > 1000) {
      lastPage= millis();
      writeString("", FONT, 1, 1, 0);
      writeString(getStringTime(false), 16, 1, 1, 1);

      writeString("AM", 60, 1, 1, 1);
    }
  }
...
boolean udp, usb, bluetooth, wifi, connecting, connected, online, loading;
...
class Facebook {
	...
	public JSONObject runPictureChoreo(String profileId) {
	...
	public JSONObject runUserchoreo() {
...
class Profile {
  ...
  public ArrayList<String> getPages() {
    ...
    pages.add(createString("Hi "+givenName, TICKER, 0, 0, 20));
    pages.add(createString(id+" "+objectType, TICKER, 1, 0, 20));
    pages.add(createString(email, TICKER, 0, 0, 20));
    pages.add(createString("+"+minAge+" "+"speaks "+language, TICKER, 1, 0, 20));
    return pages;
  }
  ...
  public ArrayList<String> getPages() {
    ...
    for (Contact thisContact : contactList) {
      if (thisContact.title != null && thisContact.title != "") {
        pages.add(createString(cleanUp(thisContact.url, true), IMAGE, 0, 0, 0));
...
public void beginComm() {
  ...
  for (int i=0; i<Serial.list().length; i++) {
    if (Serial.list()[i].indexOf(usb ? "1421" : "teleobject") != -1) {
...
public void displayPilots() {
  ...
  if (facebook.img != null) setPilot("facebook", facebook.img);
...
class Weather {
  ...
  public ArrayList<String> getPages() {
    ...
    if (!updated) {
      ...
      pages.add(createString("", BLANK, 0, 0, 0));
      pages.add(createString(cleanUp("IT'S "+nf(metric ? getCelcius(temp) : temp, 0, 1) + (metric ? PApplet.parseChar(DEGREE)+"c" : PApplet.parseChar(29)+"f"), false), CENTERED, 0, 0, 10));
      pages.add(createString(cleanUp(PApplet.parseInt(humidity)+"% humid", false), CENTERED, 0, 0, 8));
      pages.add(createString(cleanUp("pressure "+PApplet.parseInt(pressure)+"mPa", false), CENTERED, 0, 0, 8));
      pages.add(createString(cleanUp(PApplet.parseInt(clouds)+"% cloudy", false), CENTERED, 0, 0, 8));
      pages.add(createString(cleanUp("wind "+PApplet.parseInt(windSpeed) +"m/h "+(int)windDeg+PApplet.parseChar(DEGREE)+" "+getHeading(windDeg), false), CENTERED, 0, 0, 10));
...
public String getCoordinate(double coordinate, boolean isLat) {
  ...
  // println(cleanUp(degrees+""+ DEGREE + nf(minutes, 2, 0) + SINGLE_QUOTE + nf(seconds, 2, 0) + DOUBLE_QUOTE +""+hemisphere));
...
public float attract(float val, float target, float ratio, float snap) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "tele_comments_java" };
    if (passedArgs != null) {

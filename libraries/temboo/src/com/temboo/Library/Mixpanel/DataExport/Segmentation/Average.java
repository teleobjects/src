...
*/
public class Average extends Choreography {

	/**
	Create a new instance of the Average Choreo. A TembooSession object, containing a valid
	set of Temboo credentials, must be supplied.
	*/
	public Average(TembooSession session) {
		super(session, TembooPath.pathFromStringNoException("/Library/Mixpanel/DataExport/Segmentation/Average"));
	}
	...
	*/
	...
	@param String - (required, string) The expression to average per unit time. The result of the expression should be a numeric value (e.g., properties["time"]. See Choreo description for examples.
	*/
	...
	*/
	...
	@param String - (optional, string) An expression to filter events by  (e.g., properties["time"] >= 2000). See Choreo description for examples.
	*/
	 ...
	 */
	@Override
	public AverageResultSet run() {
		JSONObject result = super.runWithResults();
		return new AverageResultSet(result);
	}

...
public class Numeric extends Choreography {
	...
	*/
	public Numeric(TembooSession session) {
		super(session, TembooPath.pathFromStringNoException("/Library/Mixpanel/DataExport/Segmentation/Numeric"));
	}
	...
	*/
	...
	@param Integer - (required, integer) The number of buckets that you wish to divide the numeric values up into. Bucket ranges are automatically computed based on the maximum and minimum of the 'On' expression.
	*/
	public void setBuckets(Integer value) {
		this.inputs.setInput("Buckets", value);
	}

	/** 
	Set the value of the Buckets input for this Choreo as a String. 

	@param String - (required, integer) The number of buckets that you wish to divide the numeric values up into. Bucket ranges are automatically computed based on the maximum and minimum of the 'On' expression.
	*/
	...
	*/
	...
	@param String - (required, string) The property expression to segment the event on. Must be a numeric expression (e.g., number(properties["time"]). See Choreo description for examples.
	*/
	...
	*/
	 ...
	 */
	@Override
	public NumericResultSet run() {
		JSONObject result = super.runWithResults();
		return new NumericResultSet(result);
	}

class Ticker extends Teleobject {
  ...
  void printPages() {
    ...
    case STATUS:
      ...
      if (geolocation.formattedAddress != null) {
      ...
      if (!weather.updated) {
        ...
        pages.add(new Page("looks like it's "+int(metric ? getCelcius(weather.temp) : weather.temp) + " degrees " +(metric ? "celsius" : "farenheit") + " out there", TICKER, 0, 0, 25, sp, sp));
